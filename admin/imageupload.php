...
if (isset($_POST["action"])) {
  ...
  ';
    ...
    <tr>
     <td>' . $row["id"] . '</td>
     <td>
      <img src="data:image/jpeg;base64,' . base64_encode($row['img']) . '" height="60" width="75" class="img-thumbnail" />
     </td>
  ...
  if ($_POST["action"] == "insert") {
    ...
    $query = "INSERT INTO images(img) VALUES ('$file')";
  ...
  if ($_POST["action"] == "update") {
    $file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
    $query = "UPDATE images SET img = '$file' WHERE id = '" . $_POST["image_id"] . "'";
    if (mysqli_query($conn, $query)) {
